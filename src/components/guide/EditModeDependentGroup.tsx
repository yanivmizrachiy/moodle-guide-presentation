...
import { EDIT_MODE_DEPENDENCY_LABEL, EDIT_MODE_DEPENDENT_ACTIONS } from '@/data/guideDeck';
 ...
 * so the group works as the chapter's index instead of a static list. What a
 * row says is the action's own name, authored on that slide (`actionLabel`),
 * never a label assembled here out of the slide's question title.
 */
export function EditModeDependentGroup({ onSelect }: { onSelect: (slideId: string) => void }) {
  if (EDIT_MODE_DEPENDENT_ACTIONS.length === 0) return null;
  ...
  return (
    ...
    >
      ...
      <ul className="mt-3 grid gap-2.5 sm:grid-cols-2">
        {EDIT_MODE_DEPENDENT_ACTIONS.map((action) => (
          <li key={action.slideId}>
            <button
              type="button"
              onClick={() => onSelect(action.slideId)}
