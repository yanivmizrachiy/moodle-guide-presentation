...
try {
  rows = await runSql(
    credential.value,
    `SELECT day::text AS day, sessions, unique_browsers, slide_views, active_ms
...
console.log('════════════════════════════════════════════');
if (todayRow) {
  console.log(`  היום נכנסו למדריך:  ${int(todayRow.unique_browsers)}`);
  console.log(`  ביקורים היום:       ${int(todayRow.sessions)}`);
  console.log(`  שקפים שנצפו היום:   ${int(todayRow.slide_views)}`);
...
if (rows.length) {
  ...
  console.log('  תאריך     נכנסו   ביקורים   שקפים   דקות');
  ...
  for (const row of rows) {
    ...
    console.log(
      '  ' +
        label.padEnd(10) +
        String(int(row.unique_browsers)).padEnd(8) +
        String(int(row.sessions)).padEnd(10) +
        String(int(row.slide_views)).padEnd(8) +
        minutes(row.active_ms)
    );
  }
  const totalBrowsers = rows.reduce((sum, row) => sum + int(row.unique_browsers), 0);
  console.log(`\n  סך הכל בתקופה: ${totalBrowsers} כניסות (ספירה לפי יום, אותו דפדפן בכמה ימים נספר בכל יום).`);
}

console.log(
  '\nהמספר סופר דפדפנים, לא אנשים: אותו מורה מהטלפון ומהמחשב נספר פעמיים,\n' +
    'ומי שניקה את נתוני הדפדפן נספר מחדש. המדידה פועלת רק באתר החי.\n'
);
